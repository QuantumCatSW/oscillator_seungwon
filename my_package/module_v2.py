...
### plotters ###################################################################################################
################################################################################################################
def plot_wignerfunction(state, xvec= np.linspace(-3, 3, 100), pvec= np.linspace(-3, 3, 100), ax=None):
    """
    Plot the Wigner function of a quantum state in the x-p phase space.

    Parameters
    ----------
    state : Qobj
        The quantum state for which the Wigner function is to be computed.
    xvec, pvec : array_like, optional
        Vectors defining the x and p axes in the phase space. 
        Default is from -3 to 3 with 100 points.
    ax : matplotlib.axes.Axes, optional
        The axes on which to plot the Wigner function. 
        If None, a new figure and axes will be created.

    Returns
    -------
    ax : matplotlib.axes.Axes
        The axes containing the Wigner function plot.
    """

    W = wigner(state, xvec, pvec)
    X, P = np.meshgrid(xvec, pvec)  # Create a meshgrid for contour plotting
...
def grid_center_list_generator(x_range, y_range, minor_grid_spacing, polar=False):
    """
    Generate a list of grid center points within specified ranges.
    Parameters
    ----------
    x_range, y_range : tuple
        The ranges for the x and y coordinates, respectively, specified as (min, max).
    minor_grid_spacing : float
        The spacing between minor grid lines.
    polar : bool, optional
        If True, returns polar coordinates (radius, angle) instead of Cartesian coordinates.
    Returns

    """
    # Calculate grid centers
    x_centers = np.arange(x_range[0] + minor_grid_spacing/2, x_range[1], minor_grid_spacing)
    y_centers = np.arange(y_range[0] + minor_grid_spacing/2, y_range[1], minor_grid_spacing)

    # Generate mesh grid of center points
